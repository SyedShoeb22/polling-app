...
class Poll(models.Model):
    """
    Represents a poll question. This is equivalent to your 'polls' table.
    """
    # The 'id' primary key is created automatically by Django.
    ...
    # We use choices to enforce the poll type
    POLL_TYPE_CHOICES = [
        ('dynamic', 'Dynamic'),
        ('yesno', 'Yes/No'),
    ]
    ...
    def __str__(self):
        # A helpful representation of the object
        return self.question

class PollOption(models.Model):
    """
    Represents a single choice/option for a Poll.
    This is equivalent to your 'options' table.
    """
    # This is the magic! It creates a relationship to the Poll model.
    # on_delete=models.CASCADE means if a Poll is deleted, all its Options are also deleted.
    # This is exactly what your Flask app's "ON DELETE CASCADE" in the schema did.
    poll = models.ForeignKey(Poll, on_delete=models.CASCADE, related_name='options')

    option_text = models.CharField(max_length=200)

    def __str__(self):
        return f"{self.option_text} (for poll: {self.poll.question[:20]}...)"
class PollVote(models.Model):
    """
    Records a single vote cast by a user for a specific poll option.
    This acts as a log and is the new source of truth for all voting data.
    """
    # The user who cast the vote. If a user is deleted, their votes are also deleted.
    ...
    # A direct link to the Poll. This is crucial for the database constraint below
    # and makes querying for all votes in a poll very efficient.
    ...
    class Meta:
        # This is the most important part! This database constraint ensures that a
        # user can only vote ONCE per poll. It prevents duplicate vote records
        # for the same user on the same poll, guaranteeing data integrity.
        unique_together = ('user', 'poll')

    def __str__(self):
        # A helpful representation for the Django admin and debugging.
        return f"'{self.user.username}' voted for '{self.option.option_text}' in poll '{self.poll.question[:30]}...'"
